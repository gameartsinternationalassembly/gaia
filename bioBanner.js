  ...
  fetch('bios.json')
    ...
    .then(bios => {
      ...
      filteredBios.forEach((bio, index) => {
        ...
        participantLink.textContent = `${bio.title} (${bio.country})`;
        participantsList.appendChild(participantLink);
        participantsList.appendChild(document.createTextNode(', '));
      });

      // Remove the last comma and space from the participants list
      if (participantsList.lastChild) {
